...
from ikol.datasets import MixDataset, MixDataset_add3dpw, PW3D_mmloader, H36mSMPL_mmloader, MixDataset_MMStyle, build_dataloader
...
def train(opt, train_loader, m, criterion, optimizer, writer):
    ...
    for i, data_batch in enumerate(train_loader):
        ...
        inps = data_batch['img']
        img_metas = data_batch.pop('img_metas')
        ...
        for k, _ in data_batch.items():
            data_batch[k] = data_batch[k].cuda(opt.gpu)

        trans_inv = data_batch.pop('trans_inv')
        intrinsic_param = data_batch.pop('intrinsic_param')
        root = data_batch.pop('joint_root')
        depth_factor = data_batch.pop('depth_factor')

        # torch.autograd.set_detect_anomaly(True)
        output = m(inps, trans_inv, intrinsic_param, root, depth_factor, None)

        loss_dict = criterion(output, data_batch)
        ...
        if output_theta:
            ...
            label_masks_29 = data_batch['target_weight_29']
            label_masks_17 = data_batch['target_weight_17']

            if pred_uvd_jts.shape[1] != data_batch['target_uvd_29'].shape[1]:
                pred_uvd_jts = pred_uvd_jts.cpu().reshape(pred_uvd_jts.shape[0], 24, 3)
                gt_uvd_jts = data_batch['target_uvd_29'].cpu().reshape(pred_uvd_jts.shape[0], 29, 3)[:, :24, :]
                ...
                acc_uvd_29 = calc_coord_accuracy(pred_uvd_jts.cpu(), data_batch['target_uvd_29'].cpu(), label_masks_29.cpu(), hm_shape, num_joints=29)
            acc_xyz_17 = calc_coord_accuracy(pred_xyz_jts_17.cpu(), data_batch['target_xyz_17'].cpu(), label_masks_17.cpu(), hm_shape, num_joints=17, root_idx=root_idx_17)
...
def main_worker(gpu, opt, cfg):
    ...
    gt_val_dataset_h36m = H36mSMPL_mmloader(
        cfg=cfg,
