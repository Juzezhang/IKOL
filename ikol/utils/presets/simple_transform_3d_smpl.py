...
class SimpleTransform3DSMPL_mmloader(object):
    """Generation of cropped input person, pose coords, smpl parameters.
    ...
    def test_transform(self, src, bbox):
        ...
        # intrinsic_param[0, 0] = inp_h
        # intrinsic_param[1, 1] = inp_w
        # intrinsic_param[0, 2] = inp_h/2
        # intrinsic_param[1, 2] = inp_w/2
        # depth_factor = np.array([2.]).astype(np.float32)
        # joint_root = np.array([1000,1000,1000]).astype( np.float32)
        # joint_root = label['root_cam'].astype(np.float32) if 'root_cam' in label.keys() else np.zeros((3)).astype(
        #     np.float32)
        ...
        # trans_inv = to_tensor(trans_inv)
        # intrinsic_param = to_tensor(intrinsic_param)
        # depth_factor = to_tensor(depth_factor)
        # joint_root = to_tensor(joint_root)
        # img = im_to_torch(img)
        # # mean
        # img[0].add_(-0.406)
        # img[1].add_(-0.457)
