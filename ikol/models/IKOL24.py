...
def vis_keypoints(img, kps, kps_lines, kp_thresh=0.4, alpha=1,thickness = 2):
...
class IKOL24(nn.Module):
    ...
    def _make_deconv_layer(self):
        ...
        # deconv_layers.append(nn.ReLU(inplace=False))
    ...
    def uvd_to_uvd(self, uvd_jts, trans_inv, intrinsic_param, joint_root, depth_factor, return_relative=True):
        assert uvd_jts.dim() == 3 and uvd_jts.shape[2] == 3, uvd_jts.shape
        uvd_jts_new = uvd_jts.clone()
        assert torch.sum(torch.isnan(uvd_jts)) == 0, ('uvd_jts', uvd_jts)
    ...
    def forward(self, x, trans_inv, intrinsic_param, joint_root, depth_factor, flip_item=None, flip_output=False, image_parm=None, efficient=True):
        ...
        output = self.smpl.IKOL(
            pred_xyz_jts_24.type(self.smpl_dtype) * 2,
            betas=pred_shape.type(self.smpl_dtype),
            pred_phi=pred_phi.type(self.smpl_dtype),
            pred_leaf=pred_leaf.type(self.smpl_dtype),
            efficient=efficient
        )
        ...
        # pred_xyz_jts_17 = output_hybrik.joints_from_verts.float() / 2
        # pred_theta_mats = output.rot_mats.float().reshape(batch_size, 24 * 4)
        pred_theta_mats = output.rot_mats
    ...
    def forward_gt_theta(self, gt_theta, gt_beta):
        ...
        output = self.smpl(
            pose_axis_angle=gt_theta.type(self.smpl_dtype),
            betas=gt_beta.type(self.smpl_dtype),
            global_orient=None,
            return_verts=True
        )
