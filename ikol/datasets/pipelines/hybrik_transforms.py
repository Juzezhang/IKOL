...
from ikol.core.conventions.keypoints_mapping import get_flip_pairs
from ikol.utils.demo_utils import box2cs, xyxy2xywh
from ikol.utils.bbox import _box_to_center_scale, _center_scale_to_box
from ikol.utils.transforms import (addDPG, affine_transform, flip_joints_3d, flip_thetas, flip_xyz_joints_3d,
...
class RandomOcclusion:
    ...
    def __call__(self, results):
        ...
        if results['ann_info']['dataset_name'] != 'mpi_inf_3dhp':
...
@PIPELINES.register_module()
class HybrikImgProcess:
    def __init__(self, color_factor=0.2):
...
class HybrikLoadimg(object):
    ...
    def __call__(self, results):
        ...
        img = mmcv.imfrombytes(img_bytes, flag=self.color_type)[:,:,::-1]
...
class HybrikPipeline:
    ...
    def __call__(self, results):
        ...
        if self.two_d:
            bbox = list(results['bbox'])
            joint_img = results['joint_img'].copy()
            joints_vis = results['joint_vis'].copy()
            ...
            imgwidth, imght = results['width'], results['height']
