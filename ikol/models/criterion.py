...
class L1LossDimSMPL(nn.Module):
    def __init__(self, ELEMENTS, size_average=True, reduce=True):
        ...
        # self.output_theta = self.elements['OUTPUT_THETA']
        self.leaf_indices = [15, 22, 23, 10, 11]
    ...
    def forward(self, output, labels):
        ...
        if isinstance(output.pred_theta_mats,list):
            loss_theta = self.criterion_smpl(output.pred_theta_mats[0] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight']) + \
                         self.criterion_smpl(output.pred_theta_mats[1] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight'])
            # loss_theta = self.criterion_smpl(output.pred_theta_mats[-1] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight']) + \
            #              self.criterion_smpl(output.pred_theta_mats[-2] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight']) + \
            #              self.criterion_smpl(output.pred_theta_mats[-3] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight']) + \
            #              self.criterion_smpl(output.pred_theta_mats[-4] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight']) + \
            #              self.criterion_smpl(output.pred_theta_mats[-5] * smpl_weight * labels['target_theta_weight'], labels['target_theta'] * smpl_weight * labels['target_theta_weight'])
        else:
        ...
        loss_leaf = 0. * loss_theta
        loss = loss_beta * self.beta_weight + loss_twist * self.twist_weight + loss_uvd * self.uvd24_weight + loss_theta * self.theta_weight
        return dict(total_loss=loss, loss_beta=loss_beta, loss_twist=loss_twist, loss_uvd=loss_uvd, loss_theta=loss_theta, loss_leaf=loss_leaf)
